...
def gerar_site_vendas_completo():
    ...
    <script>
        ...
        function aplicarCupom() {{
            ...
            if(code === 'BRUNA5') {{
                cupomAtivo = {{ nome: 'BRUNA5', desc: 0.05 }};
                alert("Cupom BRUNA5 aplicado! 5% de desconto.");
            }}
            else if(code === 'DANI5') {{
                cupomAtivo = {{ nome: 'DANI5', desc: 0.05 }};
                alert("Cupom DANI5 aplicado! 5% de desconto.");
            }} 
            else if(code === 'GILMARA5') {{
                cupomAtivo = {{ nome: 'GILMARA5', desc: 0.05 }};
                alert("Cupom GILMARA5 aplicado! 5% de desconto.");
            }}
            else if(code === 'DAFNE10') {{
                cupomAtivo = {{ nome: 'DAFNE10', desc: 0.10 }};
                alert("Cupom DAFNE10 aplicado! 10% de desconto.");
            }}
            else {{
                cupomAtivo = null;
        ...
        function enviarPedido() {{
            ...
            if(cupomAtivo) msg += "%0A🏷️ *CUPOM APLICADO:* " + cupomAtivo.nome + " (-R$ " + desc.toFixed(2) + ")";
            if(freteV > 0) msg += "%0A🚚 *FRETE:* " + freteD;
            else msg += "%0A🚚 *FRETE:* Não calculado/Retirada";
            
            msg += "%0A*TOTAL GERAL: R$ " + (subtotalItens - desc + freteV).toFixed(2) + "*";
    ...
    print(f"Sucesso! Site gerado com os cupons BRUNA5, DANI5, GILMARA5 e DAFNE10.")
